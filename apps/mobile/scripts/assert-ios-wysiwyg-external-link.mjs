...
import {
  ...
  nativeWysiwygExternalLinkLogPrefix,
  parseNativeWysiwygExternalLinkProofs,
} from '../src/qa/nativeWysiwygExternalLinkProbe.ts'
...
function printHelp() {
  console.log(`Assert native iOS Simulator WYSIWYG external-link save and remove behavior.

Usage:
  node apps/mobile/scripts/assert-ios-wysiwyg-external-link.mjs [options]
...
Options:
  ...
  --wait <ms>       Delay after opening a URL before collecting logs. Defaults to 6000.
  --help            Show this help.
`)
}
...
function readConfig(args) {
  return {
    ...
    waitMs: Number(readOption(args, '--wait', '6000')),
  }
}
...
async function openProbeUrl(device, openUrl, waitMs) {
  assertNativeQaOpenUrl(openUrl, 'Native iOS WYSIWYG external-link proof')
  terminateExpoGo(device)
  await sleep(500)
  openSimulatorUrl(device, externalLinkProbeUrl(openUrl))
...
function externalLinkProbeUrl(openUrl) {
  return appendQueryParam(appendQueryParam(openUrl, 'wysiwygExternalLinkProbe', '1'), 'qaRun', Date.now().toString())
}
...
function collectSimulatorLogs(device, { last, start }) {
  return run('xcrun', [
    ...
    'compact',
    '--predicate',
    `eventMessage CONTAINS "${nativeWysiwygExternalLinkLogPrefix}"`,
  ])
}
...
async function main() {
  ...
  const { failures } = await waitForNativeProof({
    assertProofs: assertNativeWysiwygExternalLinkProofs,
    collectLogs: () => collectSimulatorLogs(device, {
      last: config.last,
      start: logStart,
    }),
    parseProofs: parseNativeWysiwygExternalLinkProofs,
    timeoutMs: config.openUrl ? proofPollTimeoutMs : 0,
  })
  if (failures.length > 0) {
    throw new Error(`Native WYSIWYG external-link proof failed:\n${formatNativeWysiwygExternalLinkFailures(failures)}`)
  }

  console.log('Native iOS WYSIWYG external-link proof passed.')
}
