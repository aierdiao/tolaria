...
} from '../src/qa/nativeMobileActionAdapterProof.ts'

const defaultExpoGoBundleId = 'host.exp.Exponent'
const defaultLogWindow = '90s'
const proofPollTimeoutMs = 16000

function printHelp() {
  console.log(`Assert native iOS Simulator mobile action adapters.

Usage:
  node apps/mobile/scripts/assert-ios-mobile-action-adapters.mjs [options]
...
async function openProbeUrl(device, openUrl, waitMs) {
  assertNativeQaOpenUrl(openUrl, 'Native iOS mobile action-adapter proof')
  terminateExpoGo(device)
  await sleep(500)
  openSimulatorUrl(device, actionAdapterProbeUrl(openUrl))
...
function actionAdapterProbeUrl(openUrl) {
  return appendQueryParam(appendQueryParam(openUrl, 'mobileActionAdapterProbe', '1'), 'qaRun', Date.now().toString())
}
...
function collectSimulatorLogs(device, { last, start }) {
  return run('xcrun', [
    ...
    'compact',
    '--predicate',
    `eventMessage CONTAINS "${nativeMobileActionAdapterLogPrefix}"`,
  ])
}
...
async function main() {
  ...
  const { failures } = await waitForNativeProof({
    assertProofs: assertNativeMobileActionAdapterProofs,
    collectLogs: () => collectSimulatorLogs(device, {
      last: config.last,
      start: logStart,
    }),
    parseProofs: parseNativeMobileActionAdapterProofs,
    timeoutMs: config.openUrl ? proofPollTimeoutMs : 0,
  })
  if (failures.length > 0) {
    throw new Error(`Native mobile action-adapter proof failed:\n${formatNativeMobileActionAdapterFailures(failures)}`)
  }

  console.log('Native iOS mobile action-adapter proof passed.')
}
