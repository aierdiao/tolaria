...
} from '../src/qa/nativeMobileCommandPaletteProof.ts'
...
function printHelp() {
  console.log(`Assert native iOS Simulator command-palette parity.

Usage:
  node apps/mobile/scripts/assert-ios-command-palette.mjs [options]
...
async function openProbeUrl(device, openUrl, waitMs) {
  assertNativeQaOpenUrl(openUrl, 'Native iOS command-palette proof')
  terminateExpoGo(device)
  await sleep(500)
  openSimulatorUrl(device, commandPaletteProbeUrl(openUrl))
...
function commandPaletteProbeUrl(openUrl) {
  return appendQueryParam(appendQueryParam(openUrl, 'mobileCommandPaletteProbe', '1'), 'qaRun', Date.now().toString())
}
...
function collectSimulatorLogs(device, { last, start }) {
  return run('xcrun', [
    ...
    'compact',
    '--predicate',
    `eventMessage CONTAINS "${nativeMobileCommandPaletteLogPrefix}"`,
  ])
}
...
async function main() {
  ...
  const { failures } = await waitForNativeProof({
    assertProofs: assertNativeMobileCommandPaletteProofs,
    collectLogs: () => collectSimulatorLogs(device, {
      last: config.last,
      start: logStart,
    }),
    parseProofs: parseNativeMobileCommandPaletteProofs,
    timeoutMs: config.openUrl ? proofPollTimeoutMs : 0,
  })
  if (failures.length > 0) {
    throw new Error(`Native command-palette proof failed:\n${formatNativeMobileCommandPaletteFailures(failures)}`)
  }

  console.log('Native iOS command-palette proof passed.')
}
