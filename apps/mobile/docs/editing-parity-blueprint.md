...
| Workspace undo/redo (`useUndoRedo`, desktop command history boundary) | `tabletWorkspaceHistory.ts`, `mobileWorkspaceEditing.ts` restore edits | Content/frontmatter history plus exact restore replay for created/deleted notes, saved Views, Type definitions, and folders. | Keep adding structural history proofs whenever a mobile edit gains a persistent write plan. |
