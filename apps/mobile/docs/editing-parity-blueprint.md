...
| Views, filters, sort, and list display (`FilterBuilder`, saved-view YAML, note-list property controls) | `MobileViewFilterBuilder.tsx`, `MobileViewDisplayPropertiesPicker.tsx`, `MobileSortPicker.tsx`, `mobileSavedViews.ts` | Saved-view create/edit/reorder/delete, filter, sort, custom-property display, primary Inbox/All Notes display overrides, undo/redo, and large-vault evaluation checks. | Keep saved-view YAML round-tripping compatible with desktop as new fields appear. |
...
| Workspace undo/redo (`useUndoRedo`, desktop command history boundary) | `tabletWorkspaceHistory.ts`, `tabletWorkspacePathHistory.ts`, `mobileWorkspaceEditing.ts` restore edits | Content/frontmatter history plus exact restore replay for created/deleted notes, saved Views, Type definitions, folders, note moves, note filename renames, and folder subtree renames. | Keep adding structural history proofs whenever a mobile edit gains a persistent write plan. |
